...
- **Trick**: set your app to hit the endpoint on initial load so by the time the user goes to the news page the server is active and response times will be fast. The data is scrapped every minute and saved to the datbase, all calls pull direct from the database allowing for fast response times.
